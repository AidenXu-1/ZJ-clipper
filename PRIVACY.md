...
- 不做 AI 总结、改写、分类或远程语义处理；抖音语音识别仅在用户主动点击后于本机运行。
...
用户首次运行安装器后，Chrome 会登记一个只允许指定 Nomo 扩展 ID 调用的 Native Messaging 本机助手。用户主动点击「转录抖音字幕」时，Chrome 才会启动该助手并传入当前抖音播放器暴露的 HTTPS 媒体地址；完成后进程自动退出，不开放常驻本地网络端口。

助手直接从抖音媒体地址读取音频并在本机运行 Whisper；视频和识别文本不会上传到转录云端或插件开发者服务器。助手会解析媒体域名并拒绝本机、内网、链路本地和保留地址。

首次启动字幕助手时，`uv` 会从 Python 软件源下载隔离运行环境及 NVIDIA 运行库，Whisper 会从 Hugging Face 下载模型文件。模型和依赖缓存位于当前设备的 `%LOCALAPPDATA%\NomoClipper\Transcriber`，用户可随时删除。
